...
router.get('/main', function(req, res, next) {
    pool.getConnection(function(err, connection) {
        ...
        var sql2 = "SELECT idx from user_table where admin_user=0";

        connection.query(sql, function(err, rows) {
            connection.query(sql2, function(err, row) {
                if (err) console.error("err : " + err);
                console.log("!!!!!rows : " + JSON.stringify(rows));
                res.render('admin_main', { title: 'Administrator Main', row: row , rows: rows});
            });
            connection.release();
        });
    });
...
router.get('/income_range', function(req,res,next) {
  ...
  var select_hour = req.query.select_hour;
  ...
  var select_hour_a = req.query.select_hour_a;


  var only_date = String(select_year) + "-" + String(select_month) + "-" + String(select_day) + " " + String(select_hour) + ":00:00"
  var only_date_a = String(select_year_a) + "-" + String(select_month_a) + "-" + String(select_day_a) + " " + String(select_hour_a) +":00:00"

  var datas = [only_date,only_date_a];
  ...
  pool.getConnection(function (err, connection) {
    ...
    connection.query(sql, datas,function (err, rows) {
      if (err)
        console.error("err : " + err);
      console.log("날짜+시간 지정 range rows : " + JSON.stringify(rows));
      res.render('income_range', {title: '매출 현황', rows:rows, only_date:only_date, only_date_a:only_date_a});
      connection.release();
    });
  });
});
